...
*/
bool Win_QextSerialPort::open(OpenMode mode) { //-V813
...
qint64 Win_QextSerialPort::size() const {
    ...
    return static_cast<qint64>(availBytes);
}
...
qint64 Win_QextSerialPort::readData(char *data, qint64 maxSize)
{
    ...
    if (Win_ComStat.cbInQue &&
		(!ReadFile(Win_Handle, static_cast<void*>(data), static_cast<DWORD>(maxSize), &Win_BytesRead, NULL) //-V2001
        || Win_BytesRead==0)) {
        lastErr=E_READ_FAILED;
        retVal=-1;
    }
    else {
        retVal=static_cast<int>(Win_BytesRead);
    }
    UNLOCK_MUTEX();
...
qint64 Win_QextSerialPort::writeData(const char *data, qint64 maxSize)
{
    ...
    if (!WriteFile(Win_Handle, static_cast<const void*>(data), static_cast<DWORD>(maxSize), &Win_BytesWritten, NULL)) { //-V2001
        lastErr=E_WRITE_FAILED;
        retVal=-1;
    }
    else {
        retVal=static_cast<int>(Win_BytesWritten);
    }
    UNLOCK_MUTEX();
...
void Win_QextSerialPort::setParity(ParityType parity) {
    ...
    if (isOpen()) {
        Win_CommConfig.dcb.Parity=static_cast<unsigned char>(parity);
        switch (parity) {
