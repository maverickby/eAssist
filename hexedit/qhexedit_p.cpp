...
void QHexEditPrivate::setDataHighlighted(int i, bool state)
{
    _xData.setDataChanged(i, state);
    update();
}

void QHexEditPrivate::setDataHighlighted(QList<uint> data, bool state)
{
    foreach (uint item, data) {
        _xData.setDataChanged(item, state);
    }
    update();
}
